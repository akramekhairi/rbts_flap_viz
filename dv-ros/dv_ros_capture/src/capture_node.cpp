...
void CaptureNode::runDiscovery(const std::string &syncServiceName) {
	...
	const std::string syncServiceTopicCopy = syncServiceName;
	mDiscoveryPublisher = mNodeHandle->advertise<DiscoveryMessage>("/dvs/discovery", 10);
	mDiscoveryThread    = std::make_unique<std::thread>(
		[this, liveCapture, syncCamera, syncServiceTopicCopy] {
        ...
        message.syncServiceTopic   = syncServiceTopicCopy;
        // 5 Hz is enough
        ros::Rate rate(5.0);
        while (mSpinThread) {
